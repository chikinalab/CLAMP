...
grViz('
 digraph CLAMP_pipeline {
   ...
   // ---- In-memory branch ----
   DF -> cpmDF -> cpmOut -> preDF -> filtered ->
       zDF -> zscored -> svdMem -> svdRes
   svdRes -> numPC -> clampK
   svdRes -> base
   clampK -> base
   zscored -> base
   ...
   H5 -> cpmFBM -> cpmFBMOut -> preFBM -> filteredFBM ->
       zFBM -> zscoredFBM -> svdFBM -> svdResFBM
   svdResFBM -> numPC
   svdResFBM -> base
   zscoredFBM -> base
...
We compute the SVD using `rsvd::rsvd()` and infer the optimal number of latent 
variables using `num.pc()`.
...
For file-backed matrices, we use `bigstatsr::big_randomSVD()` to compute the 
SVD without loading the entire matrix into RAM.
